...
#                             "Cause-list retention cleanup deleted %s rows older than %s days",
#                             deleted,
#                             settings.CAUSELIST_RETENTION_DAYS,
#                         )
#                 except Exception:
#                     db.rollback()
#                     logger.exception("Cause-list retention cleanup failed")
...
#                         "Recycle-bin cleanup deleted %s cases older than %s days",
#                         deleted,
#                         settings.CASES_RECYCLE_BIN_RETENTION_DAYS,
#                     )
#             except Exception:
#                 db.rollback()
#                 logger.exception("Recycle-bin cleanup failed")
...
# async def shutdown_event():
#     logger.info("Lawmate API shutdown")
#     task = getattr(app.state, "daily_pdf_fetch_task", None)
#     if task:
#         task.cancel()
#         try:
#             await task
#         except asyncio.CancelledError:
#             pass
#     cleanup_task = getattr(app.state, "recycle_bin_cleanup_task", None)
#     if cleanup_task:
#         cleanup_task.cancel()
#         try:
#             await cleanup_task
