...
export default function RosterPage() {
  ...
  const loadHtml = async () => {
    ...
    try {
      ...
      // Silently degrade — PDF button remains available.
    } finally {
  ...
  const isLoading = loading || htmlLoading;
          ...
          {selectedRoster && (
            <div className="flex shrink-0 items-center gap-1">
              <Button
                ...
                size="sm"
                asChild
                className="h-7 gap-1 px-2.5 text-xs font-medium text-slate-700 hover:bg-slate-100"
              >
